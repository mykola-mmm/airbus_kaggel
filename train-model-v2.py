    ...
    for i in ship_count_total:
        for j in mask_size_total:
            ...
            if len(bin_df) >= samples_per_bin:
                if (i == 0 ) and (j == 0):
                    samples.append(bin_df.sample(n=samples_per_bin*5, replace=False))
                else:
                    samples.append(bin_df.sample(n=samples_per_bin, replace=False))
            else:
                samples.append(bin_df.sample(n=len(bin_df), replace=False))
                # adjust number of samples per bin
                samples_per_bin = (MAX_NUMBER_OF_SAMPLES - len(samples)) // ((len(ship_count_total) - i) * (len(mask_size_total) - j))
